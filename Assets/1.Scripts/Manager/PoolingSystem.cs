...
public static class PoolingSystem
{
    ...
    public class Pool
    {
        ...
        public GameObject Spawn(Transform parentTransform, Vector3 localScale, Vector3 localPosition, Quaternion rot)
        {
            while (true)
            {
                GameObject obj;
                if (_inactive.Count == 0)
                {
                    ...
                    // instantiate a whole new object.
                    obj = GameObject.Instantiate(_prefab, localPosition, rot);
                    obj.name = string.Format("{0} ({1})", _prefab.name, _nextId++);
                ...
                //Debug.LogError($"Finish spawning here: {obj.name}");
                return obj;
            }
        }

        public GameObject Spawn(Transform parentTransform, Vector3 localScale, Vector3 localPosition, Vector3 localEulerAngle)
        {
            while (true)
            {
                GameObject obj;
                if (_inactive.Count == 0)
                {
                    // We don't have an object in our pool, so we
                    // instantiate a whole new object.
                    obj = GameObject.Instantiate(_prefab, localPosition, Quaternion.identity);
                    obj.name = string.Format("{0} ({1})", _prefab.name, _nextId++);
        ...
        public T Spawn<T>(Transform parentTransform, Vector3 localScale, Vector3 localPosition, Quaternion rotation)
        {
            return Spawn(parentTransform, localScale, localPosition, rotation).GetComponent<T>();
        }
        public T Spawn<T>(Transform parentTransform, Vector3 localScale, Vector3 localPosition, Vector3 localEulerAngle)
        {
            return Spawn(parentTransform, localScale, localPosition, localEulerAngle).GetComponent<T>();
        }
    ...
    public static GameObject Spawn(GameObject prefab, Vector3 pos, Quaternion rot)
    {
        ...
        return _pools[prefab.GetInstanceID()].Spawn(null, Vector3.one, pos, rot);
    }
    ...
    //public static T Spawn<T>(T prefab, Transform parentTransform = null) where T : Component
    //{
    //    return Spawn(prefab, parentTransform, Vector3.one, Vector3.zero, Quaternion.identity);
    //}

    public static T Spawn<T>(T prefab, Transform parentTransform, Vector3 localScale, Vector3 localPosition, Quaternion rotation) where T : Component
    {
        Init(prefab.gameObject);
        return _pools[prefab.gameObject.GetInstanceID()].Spawn<T>(parentTransform, localScale, localPosition, rotation);
    }

    public static T Spawn<T>(T prefab, Transform parentTransform, Vector3 localScale, Vector3 localPosition, Vector3 localEulerAngle) where T : Component
    {
        Init(prefab.gameObject);
        return _pools[prefab.gameObject.GetInstanceID()].Spawn<T>(parentTransform, localScale, localPosition, localEulerAngle);
    }
